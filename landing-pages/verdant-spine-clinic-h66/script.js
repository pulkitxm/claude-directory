document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.mlink').forEach(l => l.addEventListener('click', () => toggle(false)));
  ...
  if (slider && prev && next) {
    ...
    prev.addEventListener('click', () => slider.scrollBy({ left: -amount(), behavior: 'smooth' }));
    next.addEventListener('click', () => slider.scrollBy({ left: amount(), behavior: 'smooth' }));
    const sync = () => {
      prev.style.opacity = slider.scrollLeft <= 4 ? '0.45' : '1';
      next.style.opacity = (slider.scrollLeft + slider.offsetWidth >= slider.scrollWidth - 4) ? '0.45' : '1';
    };
