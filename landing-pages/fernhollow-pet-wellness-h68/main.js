(function () {
  ...
  var pills = Array.prototype.slice.call(document.querySelectorAll('.nav__pills .pill'));
  var sectionMap = pills.map(function (p) {
  ...
  function syncPills() {
    ...
    sectionMap.forEach(function (m) { if (m.el.offsetTop <= y) current = m; });
  ...
  function setHeight(acc) {
    ...
    body.style.maxHeight = acc.classList.contains('is-open') ? body.scrollHeight + 'px' : '0px';
  }
  ...
  window.addEventListener('resize', function () {
    accs.forEach(function (a) { if (a.classList.contains('is-open')) setHeight(a); });
  });
