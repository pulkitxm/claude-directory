document.addEventListener('DOMContentLoaded', () => {
  ...
  const content = {
    programs: {
      ...
      c1t: 'Residential', c1l: ['Assisted Living', 'Independent Living', 'Respite Care', 'Hospice Support'],
      c2t: 'Specialized', c2l: ['Memory Excellence', 'Rehabilitative Care', 'Nursing Care', 'Wellness Center']
    },
    care: {
      ...
      c1t: 'Dining', c1l: ['Chef-Led Kitchen', 'Nutrition Planning', 'Private Dining', 'Social Lounges'],
      c2t: 'Lifestyle', c2l: ['Concierge Service', 'Cultural Outings', 'Art Therapy', 'Personal Fitness']
    }
  };
  ...
  cards.forEach(card => {
    card.addEventListener('mouseenter', () => { if (window.innerWidth >= 1024) activate(card); });
    card.addEventListener('click', () => { if (window.innerWidth < 1024) activate(card); });
  });
  ...
  faqs.forEach(item => {
    item.addEventListener('click', () => {
      ...
      faqs.forEach(o => { o.classList.remove('open'); o.querySelector('.faq-a').style.maxHeight = '0px'; });
  ...
  if (wm && mask && reveal) {
    ...
    wm.addEventListener('mouseenter', () => { ci = (ci + 1) % palette.length; reveal.style.color = palette[ci]; });
    wm.addEventListener('mouseleave', () => { mask.style.setProperty('--x', '-100%'); mask.style.setProperty('--y', '-100%'); });
  }
  ...
  document.querySelectorAll('.newsletter-form').forEach(form => {
    form.addEventListener('submit', (e) => {
      ...
      setTimeout(() => { btn.textContent = 'Welcome to Almond Hours'; setTimeout(() => { btn.textContent = txt; btn.disabled = false; form.reset(); }, 1400); }, 800);
    });
  });
});
