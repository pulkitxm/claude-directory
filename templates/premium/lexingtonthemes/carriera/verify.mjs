...
const results = [];
const screenshotDirectory = path.join(root, ".audit", "screenshots");
		...
		const state = await page.evaluate(() => {
			...
			return {
				...
				remoteAssets: assets.filter(
					(asset) =>
						asset &&
						!asset.startsWith(location.origin) &&
						!asset.startsWith("data:") &&
						!asset.includes("static.cloudflareinsights.com"),
				),
			};
		});
		if (route === "/" || (width === 390 && ["/pricing.html", "/sign-in.html", "/system-overview.html"].includes(route))) {
			const label = route === "/" ? "home" : route.slice(1, -5);
