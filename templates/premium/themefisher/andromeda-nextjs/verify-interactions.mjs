import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { chromium } from "../../../../scripts/record-demos/node_modules/playwright/index.mjs";

const root = path.dirname(fileURLToPath(import.meta.url));
const origin = process.argv[2] ?? "http://127.0.0.1:4316/templates/premium/themefisher/andromeda-nextjs";
const browser = await chromium.launch();
const results = [];

async function check(interaction, route, viewport, action) {
  const page = await browser.newPage({ viewport });
  await page.goto(`${origin}${route}`, { waitUntil: "domcontentloaded" });
  let passed = false;
  try {
    passed = await action(page);
  } catch {
    passed = false;
  }
  results.push({ interaction, passed });
  await page.close();
}

await check("mobile navigation toggle", "/", { width: 390, height: 800 }, async page => {
  await page.locator('label[for="nav-toggle"]').first().click();
  return await page.locator("#nav-toggle").isChecked() && await page.locator("#nav-menu").isVisible();
});

await check("mobile pages dropdown", "/", { width: 390, height: 800 }, async page => {
  await page.locator('label[for="nav-toggle"]').first().click();
  await page.locator("#dropdown-button").click();
  return await page.locator("#dropdown").isVisible();
});

await check("pricing yearly toggle", "/pricing.html", { width: 1280, height: 800 }, async page => {
  const toggle = page.locator(".pricing-check");
  await toggle.check();
  return await page.locator(".h3.inline-flex > span").first().textContent() === "139";
});

await check("video dialog", "/about.html", { width: 1280, height: 800 }, async page => {
  await page.locator(".play-button").first().click();
  return await page.locator(".video-dialog").isVisible() && await page.locator(".video-dialog img").isVisible();
});

await check("form local submission", "/contact.html", { width: 1280, height: 800 }, async page => {
  const form = page.locator("form").first();
  const url = page.url();
  await form.evaluate(element => element.dispatchEvent(new Event("submit", { bubbles: true, cancelable: true })));
  await page.waitForTimeout(100);
  return page.url() === url;
});

await browser.close();
const summary = { tested: results.length, passed: results.filter(result => result.passed).length };
fs.mkdirSync(path.join(root, ".audit"), { recursive: true });
fs.writeFileSync(path.join(root, ".audit/interaction-verification.json"), `${JSON.stringify({ summary, results }, null, 2)}\n`);
console.log(`Verified ${summary.passed} of ${summary.tested} interactions`);
if (summary.passed !== summary.tested) process.exitCode = 1;
