...
const origin = process.argv[2] ?? "http://127.0.0.1:4326/templates/premium/themefisher/andromeda-light-nextjs";
...
await check("mobile navigation", "/", { width: 390, height: 800 }, async page => {
  await page.locator("#navToggler").evaluate(element => element.click());
  return await page.locator("#navbarNav").evaluate(element => element.classList.contains("open"));
});
await check("video modal", "/", { width: 1280, height: 800 }, async page => {
  await page.locator("#videoThumb").evaluate(element => element.click());
  await page.waitForTimeout(400);
  return await page.locator("#videoModal").evaluate(element => element.classList.contains("active"));
});
await check("accordion", "/elements.html", { width: 1280, height: 800 }, async page => {
  const header = page.locator(".accordion-header").nth(1);
  await header.click();
  return await header.evaluate(element => element.classList.contains("active"));
});
