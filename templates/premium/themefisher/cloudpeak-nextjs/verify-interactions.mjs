...
const origin = process.argv[2] ?? "http://127.0.0.1:4173/templates/premium/themefisher/cloudpeak-nextjs";
...
await check("mobile pages dropdown", "/", { width: 390, height: 800 }, async (page) => {
  await page.click('label[for="nav-toggle"]');
  await page.click("#all-pages > .nav-link");
  return await page.locator("#all-pages-dropdown").evaluate((element) => element.classList.contains("static-open"));
});

await check("yearly pricing toggle", "/pricing/", { width: 1280, height: 800 }, async (page) => {
  await page.click('label[for="pricing-switch"]');
  return !(await page.locator("#yearly-card-container").evaluate((element) => element.classList.contains("hidden")));
});

await check("FAQ accordion", "/contact/", { width: 1280, height: 800 }, async (page) => {
  const header = page.locator(".accordion-header").first();
  await header.click();
  return await header.evaluate((element) => element.closest(".accordion")?.classList.contains("open") ?? false);
});

await check("form local submission", "/contact/", { width: 1280, height: 800 }, async (page) => {
  await page.fill("#firstName", "Taylor");
  await page.fill("#lastName", "Morgan");
  await page.fill("#email", "taylor@example.com");
  await page.fill("#subject", "CloudPeak demo");
  await page.fill("#message", "Please share more details.");
  const url = page.url();
