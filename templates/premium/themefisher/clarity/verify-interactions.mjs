...
const origin = process.argv[2] ?? "http://127.0.0.1:4318/templates/premium/themefisher/clarity";
...
await check("mobile navigation toggle", "/", { width: 390, height: 800 }, async page => {
  await page.locator("#nav-toggle").evaluate(element => element.click());
  return await page.locator("#nav-menu").evaluate(element => element.classList.contains("open"));
});

await check("pages dropdown", "/", { width: 1280, height: 800 }, async page => {
  await page.locator("[data-dropdown-toggle]").evaluate(element => element.click());
  return await page.locator("[data-dropdown-toggle]").evaluate(element => element.closest(".nav-item")?.classList.contains("open") ?? false);
});

await check("pricing yearly toggle", "/pricing.html", { width: 1280, height: 800 }, async page => {
  await page.locator(".pricing-check").evaluate(element => element.click());
  return await page.locator(".pricing-check").evaluate(element => element.closest("section")?.classList.contains("billing-yearly") ?? false);
});
...
await check("FAQ accordion", "/faq.html", { width: 1280, height: 800 }, async page => {
  ...
  return await header.evaluate(element => element.closest("details")?.open ?? false);
});
