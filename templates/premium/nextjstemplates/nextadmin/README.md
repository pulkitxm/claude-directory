...
Dark/light mode is toggled via the sun/moon button in the top header. The chosen theme is written to `localStorage` under the key used by the vanilla JS toggle, and the initial state falls back to the OS-level `prefers-color-scheme` if no preference has been saved. No server round-trip is needed because the class is applied before first paint.
...
Part of the [NextAdmin](../) collection in the [Templates](../../../) category. [Browse the live gallery](https://pulkitxm.com/claude-directory).
