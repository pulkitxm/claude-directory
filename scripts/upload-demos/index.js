...
const SKIP_DIRS = new Set(["node_modules", ".git", ".claude", ".github", "extras", "scripts"]);
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const repoPath = path.relative(REPO_ROOT, absPath).split(path.sep).join("/");
        const repoDir = repoPath.slice(0, -"/demo.mp4".length);
        videos.push({ repoDir, repoPath, absPath, publicId: publicIdFor(repoDir) });
      }
    }
  }
